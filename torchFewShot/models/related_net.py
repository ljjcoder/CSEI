...
from cam_surport import CAM,CAM_similarity
...
class RelaNet(nn.Module):
    ...
    def __init__(self,input_channel, layer_channels, kernel=3):
        ...
        for i in range(len(layer_channels)):
            if i==0:
                self.conv1=nn.Conv2d(self.input_channel, self.layer_channels[i], kernel_size=kernel, stride=1, padding=0, bias=True)
                self.bn1 = nn.BatchNorm2d(self.layer_channels[i])
            elif i==1:  
                self.conv2=nn.Conv2d(self.layer_channels[i-1], self.layer_channels[i], kernel_size=kernel, stride=1, padding=0, bias=True)
                self.bn2 = nn.BatchNorm2d(self.layer_channels[i])                
            else:
                self.conv3=nn.Conv2d(self.layer_channels[i-1], 128, kernel_size=kernel, stride=1, padding=0, bias=True)
                self.bn3 = nn.BatchNorm2d(128)
        for j in range(len(layer_channels)):
            if j==0:
                self.conv1_attention=nn.Conv2d(640, self.layer_channels[j], kernel_size=kernel, stride=1, padding=0, bias=True)
                self.bn1_attention = nn.BatchNorm2d(self.layer_channels[j])
            elif j==1:  
                self.conv2_attention=nn.Conv2d(self.layer_channels[j-1], self.layer_channels[j], kernel_size=kernel, stride=1, padding=0, bias=True)
                self.bn2_attention = nn.BatchNorm2d(self.layer_channels[j])                
            else:
                self.conv3_attention=nn.Conv2d(self.layer_channels[j-1],512, kernel_size=kernel, stride=1, padding=0, bias=True)
                #self.bn3 = nn.BatchNorm2d(self.layer_channels[i])        
        #self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)
        self.cam_support = CAM()
        self.se=SELayer(640,8)
    ...
    def forward(self, x,label_1shot):
        ...
        concat_feature=concat_feature.mean(2).reshape( -1,640,shape[3],shape[4] )
        #print(concat_feature.shape)
        #concat_feature=concat_feature.mean(2)
        #concat_feature=concat_feature.unsqueeze(2).unsqueeze(2)
        #print(concat_feature.shape)
        #concat_feature=concat_feature.reshape( 4,-1,1,1 )
        #exit(0)
        #for i in range(0):
            #if i==0:
                ...
                #concat_feature = self.relu(concat_feature)           
        #spatial=torch.sigmoid( concat_feature).reshape( shape[0],shape[1],512,1,1 )    
        #spatial=torch.sigmoid( concat_feature).reshape( shape[0],1,512,1,1 ) 
        #SELayer        
        #print(spatial.shape)#[4,5,512,1,1]
        #print(spatial[0].max(1)[0])        
        #print(spatial[0,1])
        #exit(0)
        spatial=self.se(concat_feature).unsqueeze(1)        
        concat_feature=(spatial+1)*x 
        #spatial=self.se(concat_feature).unsqueeze(1)
        #print(spatial.shape)
        #print(x.shape)
        #exit(0)
